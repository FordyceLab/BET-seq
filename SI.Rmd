...
returnDf2 =
  ...
  mutate(readTotal = log10(readTotal),
         ...
         libSize = factor(libSize, labels = c(
           expression(bold(paste("lib. = 10"^"2"))),
           expression(bold(paste("lib. = 10"^"3"))),
           expression(bold(paste("lib. = 10"^"4"))),
           expression(bold(paste("lib. = 10"^"5"))),
           expression(bold(paste("lib. = 10"^"6"))))))
...
ggsave("~/Flanks/Images/S1A.png", plot = S1A, height = 10, width = 10, units = "cm")
...
ggsave("~/Flanks/Images/S1B.png", plot = S1B, height = 10, width = 10, units = "cm")
```
...
#overlayDf %>% group_by(cc) %>% summarise(count = n()) %>% mutate(total = count*cc) %>% summarise(meanval = sum(total)/sum(count))
```