...
```{r}
right_join(fread("data/std_kd_cbf1.csv") %>% 
...
```{r}
# Held-out r^2
training_data <- fread("data/train_seqs.txt", header = FALSE) %>% 
  rename(flank = V1)
...
```{r}
fread("data/stdEpistasis1.csv", header = T) %>%
  tbl_df() %>%
...
# read data
returnDf2 =
  fread("data/modelSimulation.txt", header = T) %>%
...
```{r}
cbf1_training <- read.table("data/cbf1_model.acc", header = TRUE) %>% 
  gather(dataset, RMSE, -epoch) %>% 
  mutate(dataset = ifelse(dataset == "train_acc", "training", "validation"),
         protein = "Cbf1")

pho4_training <- read.table("data/pho4_model.acc", header = TRUE) %>% 
  gather(dataset, RMSE, -epoch) %>% 
  mutate(dataset = ifelse(dataset == "train_acc", "training", "validation"),
         protein = "Pho4")
...
pho4Curves = 
  fread("data/pho4BindingCurves.csv", sep = ",", header = T) %>%
  tbl_df() %>%
  ...
  left_join(.,fread("data/seq2ID_pho4.txt", header = F) %>%
...
cbf1Curves = 
  fread("data/cbf1BindingCurves.csv", sep = ",", header = T) %>%
  tbl_df() %>%
  rename(ID = Onum, fBound = R, Conc = Ch) %>%
  mutate(index = paste("ID",ID,sep=""), index = factor(index, paste0("ID",1:32))) %>%
  createCurves(.) %>%
  mutate(index = factor(index, levels = paste0("ID",1:32))) %>%
  left_join(.,fread("data/seq2ID_cbf1.txt", header = F) %>%
    unite("flank",V3,V5,sep="_") %>%
    select(index = V1, flank) %>%
    mutate(index = paste("ID",index,sep=""))) %>%
  mutate(flank = ifelse(index == "ID32","negative control",flank)) %>%
...
```{r}
coreDf = 
  fread("data/core_mutation_ddG.txt", header = T) %>% 
  tbl_df() %>% 
  mutate(type = "core") %>% 
...
outAdd = addMotif(fread("data/countsGF.txt", header = T) %>% 
                    na.omit() %>% 
...
```{r}
cvLasso = bind_rows(fread("data/lassoCV_pho4.txt", header = T) %>% mutate(protein = "Pho4"),
                    fread("data/lassoCV_cbf1.txt", header = T) %>% mutate(protein = "Cbf1")) %>%
  mutate(protein = factor(protein, levels = c("Pho4","Cbf1")))
...
## INCLUDE CSV: LASSO
```{r}
write.table(cvLasso, "data/SuppData/supp_data1.csv", sep = ",", row.names = F, col.names = T, quote = F)
```

## Figure S12: Cbf1 + Pho4 LASSO magnitudes
```{r}
dfVar = bind_rows(fread("data/dfVar_cbf1.txt", header =T) %>% mutate(protein = "Cbf1"),
                  fread("data/dfVar_pho4.txt", header =T) %>% mutate(protein = "Pho4")) %>%
  group_by(flank) %>%
...
refs =
  bind_rows(std_kd %>% 
            ...
            fread("data/Nat_gen_titration.txt", header = T) %>% 
              tbl_df() %>% 
              select(flank,stdE = Maerkl_ddG) %>% 
              mutate(protein = "Pho4",
                     source = "Rajkumar et al."),
            fread("data/Science_titration.txt", header = F) %>% 
...
```{r}
rateVal = 
  fread("data/transcriptionRate.txt", header = F, sep = "\t", stringsAsFactors = F) %>%
...
expVal = 
  fread("data/NARexpression.txt", header = T, sep = "\t") %>%
...
```{r}
shockDf = 
  bind_rows(fread(paste0("data/shock_pho4.txt"), header = T) %>% mutate(protein = "pho4"),
            fread(paste0("data/shock_cbf1.txt"), header = T) %>% mutate(protein = "cbf1")) %>%
  tbl_df() %>% 
  mutate(protein = ifelse(protein == "pho4","Pho4","Cbf1"),
         protein = factor(protein, levels = c("Pho4","Cbf1")))
...
```{r}
overlayDf = 
  left_join(bind_rows(fread(paste0("data/shock_pho4.txt"), header = T) %>% mutate(protein = "pho4"),
                      fread(paste0("data/shock_cbf1.txt"), header = T) %>% mutate(protein = "cbf1")) %>%
                      tbl_df(),
