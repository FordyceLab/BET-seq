...
## Raw data
counts.df = 
  fread("data/Manuscript_Data/counts.txt", header = T) %>%
  tbl_df() 
...
## NN predictions
countsNN.df =
  fread("data/Manuscript_Data/all_predicted_ddGs.csv", header = T, sep = ",") %>%
  tbl_df() %>%
...
countsNN_scaled = fread("data/Manuscript_Data/scaled_nn_preds.txt", header = T) %>% 
  tbl_df() %>% 
  mutate(protein = ifelse(protein == "Pho4","pho4","cbf1"))

## titration data
std_kd = bind_rows(fread("data/Manuscript_Data/std_kd_pho4.csv", header = T) %>% 
                     mutate(protein = "pho4") %>%
                     slice(1:31), # remove negative control
                     fread("data/Manuscript_Data/std_kd_cbf1.csv", header = T) %>% 
                     mutate(protein = "cbf1") %>%
                     slice(1:31)) %>%
  tbl_df() %>%
...
ref.gen = read.fasta(file = "data/Manuscript_Data/S288C_reference_sequence_R27-1-1_20031001.fsa") #2003 yeast reference genome 
...
model_outputs <- fread("data/Manuscript_Data/model_outputs.txt", header = T)
