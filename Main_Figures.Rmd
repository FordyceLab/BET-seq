...
foreach(prot = c("pho4","cbf1"))%dopar%{
  ...
  write.table(shockDf, paste0("data/shock_",prot,".txt"), col.names = T, row.names = F, quote = F, sep = "\t")
}
```
...
shockDf = 
  fread(paste0("data/shock_",prot,".txt"), header = T) %>% 
...
# read-in data
readDat2 =
  fread("data/spec_R3_2.txt", header = F) %>% #R3.2 (only from Expt 1)
  tbl_df() %>%
...
write.table(dsPWM, "data/dspwmSimulation.txt", col.names = T, row.names = F, quote = F)
```


```{r}
dsPWM = fread("data/dspwmSimulation.txt", header = T)
...
write.table(returnDf, "modelSimulation.txt", col.names = T, row.names = F, quote = F)
```
...
readDat =
  fread("data/Stormo_All_Expts.txt", header = T) %>%
  tbl_df() %>%
...
write.table(dsDat, "data/downsampleSimulation.txt", col.names = T, row.names = F, quote = F)
```
```{r}
dsDat = fread("data/downsampleSimulation.txt", header = T) %>% 
  filter(type == "ref_ddG") %>% 
  mutate(r2 = r2/seqObsPct) %>% 
...
data <- fread("data/scaled_nn_preds.txt") %>% 
...
write.table(model_outputs, "data/model_outputs.txt", row.names = FALSE, col.names = TRUE, sep = "\t", quote = FALSE)
```
...
interaction_data <- fread("~/Flank_seq/data/scaled_nn_preds.txt") %>% 
...
write.table(fitLasso, paste0("data/lassoFit_",protVal,".txt"), row.names = F, col.names = T, quote = F, sep = "\t")

# 10-fold cross validation
cv.lasso=cv.glmnet(x,y, intercept=FALSE, parallel = T)
cvLasso = tidy(cv.lasso)
write.table(cvLasso, paste0("data/lassoCV_",protVal,".txt"), row.names = F, col.names = T, quote = F, sep = "\t")
...
dfVar =
  ...
  ungroup()
write.table(dfVar, paste0("data/dfVar_",protVal,".txt"), row.names = F, col.names = T, quote = F, sep = "\t")

# LASSO plot
fitLasso = bind_rows(fread(paste0("data/lassoFit_pho4.txt"), header = T) %>% mutate(protein = "pho4"),
                     fread(paste0("data/lassoFit_cbf1.txt"), header = T) %>% mutate(protein = "cbf1")) %>% 
...
write.table(lassoPlot, "data/lassoPlot.txt", col.names = T, row.names = F, quote = F, sep = "\t")
```

```{r}
labelDf2 = 
  fread("data/lassoPlot.txt", header = T) %>%
  filter(estimate != 0) %>% 
...
lassoPlot2 = 
  left_join(fread("data/lassoPlot.txt", header = T), labelDf2) %>% 
