...
#       mutate(r2seq = ifelse(typeCor == "pearson", r2seq^2, r2seq),
#              r2seq = r2seq * seqObsPct,
#              r2pwm = ifelse(typeCor == "pearson", r2pwm^2, r2pwm))
#   }
...
p1 = ggplot(dsPWM %>% filter(readTotal <= 1E6)) +
  presentation +
  ylim(c(0,1)) +
  geom_point(aes(readTotal,value,group = interaction(readTotal,key),color=key), 
             stat = "summary", 
             fun.y = "median",
             size = 0.25) +
  geom_line(aes(readTotal,value,color = key), 
            stat="summary",
            fun.y = "median", 
            size = 0.25) +
...
returnDf2 =
  ...
  mutate(key = ifelse(key == "r2", 1, 2),
         key = factor(key, labels = c(TeX("$\\textbf{Pearson's}$ $\\mathbf{r^2}$"), TeX("$\\textbf{fract. obs.}$"))),
...
p1 = ggplot(dsDat) +
  ...
  geom_vline(xintercept = 100, lty = 2, size = 0.25) +
  theme(aspect.ratio = 1/2,
        legend.text.align = 0) +
...
#write.table(outAdd, "~/Desktop/additive.txt", col.names = T, row.names = F, quote = F, sep = "\t")
...
ggsave("~/Flanks/Images/f4pB.svg", plot = p1, height = 8, width = 17.8, units = "cm")
```
